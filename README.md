...
Parsing requires the z/OS C SMF headers. During package build, `setup.py`
compiles a native parser extension and the supported SMF headers from
`/usr/include/zos`; set `SMF_PARSER_ZOS_INCLUDE` before building to use another
directory. At runtime, generic SMF header parsing uses the native extension and
