...
def _smf119_parser_lines(record_type: int) -> list[str]:
    ...
    return [
        ...
        "                }",
        "                if (smf119_ident_available >= 8 &&",
        "                    set_bytes(result, \"SMF119TI_SYSName\", "
        "smf119_ident + 0, 8) < 0) {",
        "                    Py_DECREF(result);",
        "                    return NULL;",
        "                }",
        "                if (smf119_ident_available >= 16 &&",
        "                    set_bytes(result, \"SMF119TI_SysplexName\", "
        "smf119_ident + 8, 8) < 0) {",
        "                    Py_DECREF(result);",
        "                    return NULL;",
        "                }",
        "                if (smf119_ident_available >= 24 &&",
        "                    set_bytes(result, \"SMF119TI_Stack\", "
        "smf119_ident + 16, 8) < 0) {",
        "                    Py_DECREF(result);",
        "                    return NULL;",
        "                }",
        "                if (smf119_ident_available >= 32 &&",
        "                    set_bytes(result, \"SMF119TI_ReleaseID\", "
        "smf119_ident + 24, 8) < 0) {",
        "                    Py_DECREF(result);",
        "                    return NULL;",
        "                }",
        "                if (smf119_ident_available >= 40 &&",
        "                    set_bytes(result, \"SMF119TI_Comp\", "
        "smf119_ident + 32, 8) < 0) {",
        "                    Py_DECREF(result);",
        "                    return NULL;",
        "                }",
        "                if (smf119_ident_available >= 48 &&",
        "                    set_bytes(result, \"SMF119TI_ASName\", "
        "smf119_ident + 40, 8) < 0) {",
        "                    Py_DECREF(result);",
        "                    return NULL;",
        "                }",
        "                if (smf119_ident_available >= 56 &&",
        "                    set_bytes(result, \"SMF119TI_UserID\", "
        "smf119_ident + 48, 8) < 0) {",
        "                    Py_DECREF(result);",
        "                    return NULL;",
        "                }",
        "                if (smf119_ident_available >= 54 &&",
        "                    set_long(result, \"SMF119TI_ASID\", "
        "read_unsigned_be(smf119_ident + 52, 2)) < 0) {",
        "                    Py_DECREF(result);",
        "                    return NULL;",
        "                }",
        "                if (smf119_ident_available >= 55 &&",
